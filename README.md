...
- **시간 복잡도 개선:**  
  `GetInitialTopScores()` 메서드의 기존 전체 정렬 `O(N log N)` → **페이지 기반 처리로 개선:**  
  *(N: Firebase DB 안에 저장된 “모든 점수 데이터”의 개수, K: 가져오려는 리더보드의 고유 유저 수, C: 필요한 총 페이지 수, P: 페이지당 점수 수)*  

  고유 유저 비율을 **r (0 < r ≤ 1)** 이라고 하면,  

  $$
  r = \frac{\text{고유 유저 수}}{\text{페이지당 점수 수}}
  $$

  한 페이지에서 평균적으로 `r × P` 명의 고유 유저를 얻게 됩니다.  
  따라서 **K명(목표 유저 수)** 을 얻기 위해 필요한 페이지 수는 다음과 같습니다.

  $$
  C ≈ \frac{K}{r × P}
  $$

  즉, **r이 작을수록(중복이 많을수록)** 더 많은 페이지를 탐색해야 합니다.  


  | 구분 | 설명 | 시간 복잡도 |
  |------|------|--------------|
  | **이상적인 경우 (중복 없음)** | 한 페이지마다 모두 다른 유저 | **O(K log P), r = 1** |
  | **일반적인 경우 (중복 존재)** | 한 페이지 내 일부 유저 중복 | **O((K / r) log P), 0 < r < 1** |
  | **최악의 경우** | 한 유저가 대부분 점유 | **O(C × P log P) ≈ O(N log P), r ≈ 0** *(전체 탐색 수준)* |
