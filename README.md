...
- **시간 복잡도 개선:**  
  기존 전체 정렬 `O(N log N)` → **페이지 기반 처리로 개선:**  
  *(N: Firebase DB 안에 저장된 “모든 점수 데이터”의 개수, C: 페이지 수, P: 페이지 크기)*  
  ...
  | 구분 | 설명 | 시간 복잡도 |
  |------|------|--------------|
  | **이상적인 경우 (중복 없음)** | 한 페이지마다 모두 다른 유저 | **O(K log P)** |
  | **일반적인 경우 (중복 존재)** | 한 페이지 내 일부 유저 중복 | **O((K / r) log P), r < 1** |
  | **최악의 경우** | 한 유저가 대부분 점유 *(r → 0)* | **O(C × P log P) ≈ O(N log P)** *(전체 탐색 수준)* |
