...
public class EnemyManager : MonoBehaviour
{
    ...
    public bool GenerateEnemy(SquadronMemberStruct data)
    {
        ...
        GameObject go = SystemManager.Instance.GetCurrentSceneMain<InGameSceneMain>().EnemyCacheSystem.Archive(
            FilePath, 
            new Vector3(data.GeneratePointX, data.GeneratePointY, data.GeneratePointZ),
            data.Rotation
            );

        EnemyController_Still enemyStill = go.GetComponent<EnemyController_Still>();
        enemyStill.CheckResurrection();
    ...
    public bool AddList(EnemyController enemy)
    {
        if (enemies.Contains(enemy))
            return false;
    ...
    public bool RemoveList(EnemyController enemy)
    {
        if (!enemies.Contains(enemy))
            return false;
    ...
    public List<EnemyController> GetContainEnemies(Collider collider)
    {
        List<EnemyController> contains = new List<EnemyController>();
